...
## Adding new function run_batch_stream to print answer the moment it arrives
async def run_batch_stream(questions: list[Question], fail_rate: float = 0.0) -> list[Answer]:
    """same as above but instead of gather, collect as completed"""
    tasks = [ask_llm_with_retry(q, fail_rate=fail_rate) for q in questions]
    results: list[Answer] = []
...
### running main method as suggested
